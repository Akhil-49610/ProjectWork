...
namespace gestionSalaries
{
    public partial class frm_Paie : Form
    {
        ...
        private void load_data()
        {
            sql = "SELECT DISTINCT (" +
             "p.`trans_id`" +
             "),e.emp_code as 'Code', CONCAT(  `emp_fname` ,  ' ',  `emp_lname` ,  ' ',  `emp_mname` ) AS  'Employee Name'" +
             ",  `d_rate` AS 'DailyRate' ,  `num_days` AS 'NumberOfDays' ,  `r_wage` AS 'RateSalary', "
             + " `gross_sal` AS 'Gross Salary', `total_ded` AS 'DeductionTotal',  `net_income` AS 'Net Income' ,  " +
