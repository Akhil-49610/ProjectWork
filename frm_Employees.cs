    ...
    public partial class frm_Employees : Form
    {
...

        {
            ...
            if (txtcode.Text == "" || txtfname.Text == "" || txtlname.Text == "" || txtmname.Text == ""
              ...
              || txtage.Text == "" || txtcontact.Text == "" || txtdrate.Text == "" || txtposition.Text == "")
            {
                MessageBox.Show("\r\nPlease complete all fields !", "Info", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
            else
            {
                if (rdomale.Checked)
        ...
        private void list_Employee()
        {
            sql = "SELECT `emp_code` AS 'CODE',CONCAT( `emp_fname`,' ', `emp_lname`,' ', `emp_mname`) AS 'NAME'"
             + ", `emp_age` AS 'AGE', `emp_sex` AS 'SEX', `status` AS 'STATUS', `address` AS 'ADDRESS'"
